...
public class EncryptionUtil {
    ...
    public static String encrypByMD5(String originString) {
        try {
            ...
            //将得到的字节数组变成字符串返回
            String s = byteArrayToHex(bytes).toUpperCase();
     ...
     * 对指定的字符串进行MD5加密
     */
    public static String encrypByMD5Source(String originString) {
        try {
            //创建具有MD5算法的信息摘要
            MessageDigest md = MessageDigest.getInstance("MD5");
            //使用指定的字节数组对摘要进行最后更新，然后完成摘要计算
    ...
    public static void main(String[] args){
        String testStr = "安徽省招标集团";
        String a = encrypByMD5(testStr);
        System.out.println(a);
        a = UuidUtil.randomUUID().toUpperCase();
        System.out.println(a);
    }
}